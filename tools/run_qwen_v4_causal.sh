...
RUN_ID=objective_data_causal_pilot_qwen_8b_v4_20260822
CONFIG=configs/experiments/objective_data_causal_pilot_qwen_8b_v4.yaml
MANIFEST=outputs/manifests/objective_data_causal_pilot_qwen_8b_v4_20260822/objective_data_causal_pilot_qwen_8b_v4_20260822.json
RUN_DIR=outputs/runs/objective_data_causal_pilot_qwen_8b_v4_20260822
REPORT_DIR=outputs/reports/objective_data_causal_pilot_qwen_8b_v4
AUDIT_PATH=outputs/audits/objective_data_causal_pilot_qwen_8b_v4_audit.json

# Remove only this run's exact artifacts so a retry cannot inherit a stale
# checkpoint or an incomplete phase-3 summary.
for path in \
...
task_path = bundle.project_root / str(bundle.experiment.scoring["objective_tasks_path"])
task_metadata = json.loads(task_path.read_text())
if task_metadata.get("review", {}).get("status") != "frozen":
    raise SystemExit("CONFIG_ARTIFACT_GATE_FAILED: objective dataset is not frozen")
missing = []
...
required_direction_types = {"standard", "random", "sign_reversed"}
expected = len(manifest["model_ids"]) * len(manifest["trait_ids"]) * len(required_direction_types)
if len(downstream) != expected or any(
    not job["payload"].get("alpha_values") or not job["payload"].get("direction_type")
    for job in downstream
) or direction_types != required_direction_types:
    raise SystemExit(
...
if (
    ...
    or int(facts.get("response_file_count", 0)) != 6
    or int(facts.get("selected_completed_count", 0)) != int(facts.get("selected_job_count", 0))
):
...
root = Path("outputs/reports/objective_data_causal_pilot_qwen_8b_v4/phase3")
...
rm -rf /tmp/objective_qwen_v4_causal_stage
stage=/tmp/objective_qwen_v4_causal_stage
mkdir -p "$stage"/{manifests,runs,reports,configs,models,data,audits}
cp "$MANIFEST" "$stage/manifests/"
cp -r "$RUN_DIR" "$stage/runs/"
cp -r outputs/reports/objective_data_causal_pilot_qwen_8b_v4 "$stage/reports/"
...
cp outputs/audits/objective_data_causal_pilot_qwen_8b_v4_audit.json "$stage/audits/"
...
stage = Path("/tmp/objective_qwen_v4_causal_stage")
prefix = "experiments/objective_data_causal_pilot_qwen_8b_v4_20260822"
repo = "ic-org/deconfounding-interp-shared"
api.upload_folder(
    ...
    repo_id=repo,
    repo_type="dataset",
    commit_message="Add Qwen v4 objective causal pilot 2026-08-22",
    token=True,
)
entries = list(
    api.list_repo_tree(
...
for path in \
  ...
  data/interim/geometry_null_repair_8b \
  outputs/directions/geometry_null_repair_8b \
  outputs/manifests/objective_data_causal_pilot_qwen_8b_v4_20260822 \
  outputs/runs/objective_data_causal_pilot_qwen_8b_v4_20260822 \
  outputs/reports/objective_data_causal_pilot_qwen_8b_v4 \
  outputs/audits/objective_data_causal_pilot_qwen_8b_v4_audit.json; do
  rm -rf -- "$path"
done

echo QWEN_V4_CAUSAL_DONE
