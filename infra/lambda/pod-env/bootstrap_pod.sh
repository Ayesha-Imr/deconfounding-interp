...
# Keep uv's managed interpreter on the persistent filesystem too. Otherwise a
# venv's python symlink points into the previous pod's ephemeral home and every
# new pod has to download/recreate the interpreter before it can run a job.
UV_PYTHON_INSTALL_DIR="${UV_PYTHON_INSTALL_DIR:-$WORKSPACE_DIR/uv_python}"
...
echo "uv Python (persistent):  $UV_PYTHON_INSTALL_DIR"
echo
