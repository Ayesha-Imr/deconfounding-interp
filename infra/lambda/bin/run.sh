...
# Send the command as stdin to a non-login shell. Passing a multiline command
# as `ssh host bash -lc "..."` lets OpenSSH re-parse the string and can emit
# `bash: -c: option requires an argument` when the command contains exports or
# command substitutions. Keeping the command in stdin preserves its exact
# quoting (including one-session credentials) and makes failures visible.
{
    printf '%s\n' 'set -euo pipefail'
