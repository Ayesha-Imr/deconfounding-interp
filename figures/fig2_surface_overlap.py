...
def plot(data_dir: Path, output_dir: Path) -> None:
    ...
    ax.axvline(
        ...
        linestyle=":",
        zorder=1,
        label=f"Random baseline ({random_mean:.1%})",
    )
    ...
    for i, trait in enumerate(traits):
        ...
        rightmost = max(
            avg.loc[avg["trait_id"] == trait, "mean_overlap"].values[0], trait_rows.max()
        )
    ...
    ax.set_yticks(y_pos)
    ax.set_yticklabels(
        [f"{TRAIT_DISPLAY[t]}  ({TRAIT_TYPE[t][:1].upper()})" for t in traits], fontsize=10
    )
